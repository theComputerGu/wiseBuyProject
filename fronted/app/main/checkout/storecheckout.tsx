...
export default function StoreCheckoutScreen() {
  ...
  const missingProducts = useMemo(() => {
    return items.filter(
      (item) =>
        !products.some((p) => p.itemcode === item._id.itemcode)
    );
  }, [items, products]);
  ...
  const handleCheckout = async () => {
    ...
    const purchasedItems = products.map((p) => ({
      itemcode: p.itemcode,
      quantity: p.amount,
    }));
    ...
    <SafeAreaView style={styles.wrapper}>
      <View style={styles.container}>
        ...
        <ScrollView>
          ...
          {missingProducts.length > 0 && (
            <>
              <ItimText size={16} weight="bold" color="#999">
                מוצרים שלא זמינים בחנות
              </ItimText>
              {missingProducts.map((m) => (
                <CheckoutCard
        ...
        <Pressable style={styles.checkoutBtn} onPress={handleCheckout}>
          <MaterialCommunityIcons name="cart-check" size={20} color="#fff" />
          <ItimText size={16} color="#fff" weight="600">
            אישור קניה • ₪{totalPrice}
          </ItimText>
        </Pressable>
